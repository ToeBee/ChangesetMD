...
class ChangesetMD:
    ...
    def doReplication(self, connection):
        ...
        try:
            ...
            print(
                'error getting lock on state table. Another process might be running'
            )
            return 1
        ...
        try:
            ...
            try:
                print(
                    (
                        'latest sequence on OSM server: '
                        + str(lastServerSequence)
                    )
                )
                if lastServerSequence > lastDbSequence:
                    ...
                    while currentSequence <= lastServerSequence:
                        self.parseFile(
                            connection,
                            self.fetchReplicationFile(currentSequence),
                            True,
                        )
                        cursor.execute(
